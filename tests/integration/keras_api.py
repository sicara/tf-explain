...
def test_all_keras_api(model_builder, mnist_dataset, validation_dataset, num_classes):
    ...
    callbacks = [
        tf_explain.callbacks.GradCAMCallback(
            validation_data, "grad_cam_target", class_index=target_class
        ),
        tf_explain.callbacks.ActivationsVisualizationCallback(
            validation_data, "grad_cam_target"
        ),
        tf_explain.callbacks.SmoothGradCallback(
            validation_data, class_index=target_class, num_samples=15, noise=1.0
        ),
    ]
