...
def sequential_api_model(num_classes):
    model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), activation='relu', name='grad_cam_target'),
        tf.keras.layers.MaxPool2D(pool_size=(2, 2)),
...
def subclassing_api_model(num_classes):
    class SubclassedModel(tf.keras.models.Model):
        def __init__(self, name='subclassed'):
            ...
            self.conv_2 = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), activation='relu')
            self.maxpool_1 = tf.keras.layers.MaxPool2D(pool_size=(2, 2))

            self.conv_3 = tf.keras.layers.Conv2D(filters=32, kernel_size=(3, 3), activation='relu')
            self.conv_4 = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), activation='relu', name='grad_cam_target')
        ...
        def call(self, inputs, **kwargs):
            x = inputs
            for layer in [self.conv_1, self.conv_2, self.maxpool_1, self.conv_3, self.conv_4, self.maxpool_2, self.flatten, self.dense_1, self.dense_2]:
                x = layer(x)
    ...
    model(np.random.random([4, *INPUT_SHAPE]).astype('float32'))  # Sample call to build the model
...
def test_all_keras_api(model_builder, mnist_dataset, validation_dataset, num_classes):
    ...
    callbacks = [
        tf_explain.callbacks.GradCAMCallback(validation_data, 'grad_cam_target', class_index=target_class),
        tf_explain.callbacks.ActivationsVisualizationCallback(validation_data, 'grad_cam_target'),
        tf_explain.callbacks.SmoothGradCallback(validation_data, class_index=target_class, num_samples=15, noise=1.),
    ]
