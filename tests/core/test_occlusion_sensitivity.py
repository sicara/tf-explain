...
def test_should_get_confidence_for_patch(convolutional_model, mocker):
    ...
    mocker.patch('tf_explain.core.occlusion_sensitivity.apply_grey_patch', return_value=np.random.randint(low=0, high=255, size=convolutional_model.inputs[0].shape[1:]))

    class_index = 0
    confidence = OcclusionSensitivity.get_confidence_for_random_patch(convolutional_model, None, class_index, None, None, None)

    assert confidence == 1


def test_should_produce_heatmap(convolutional_model, random_data, mocker):
    mocker.patch('tf_explain.core.occlusion_sensitivity.grid_display', return_value=mocker.sentinel.grid)
